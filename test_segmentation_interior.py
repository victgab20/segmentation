# dice_segmentation_interior_topN.py
...
media_dice = df.groupby('Arquivo')['Dice_SegmentationInterior'].mean()
...
for n in range(2, 11):
    ...
    for pasta in sorted(df['Pasta'].unique(), key=lambda x: int(x.replace('Point', ''))):
        ...
        try:
            mask_seg_interior = io.imread(os.path.join(folder_path, 'SegmentationInterior.tif'))
        ...
        if bin_comb.shape != mask_seg_interior.shape:
            continue

        dice_interior = dice_score(bin_comb, mask_seg_interior)

        resultados.append({
            'Pasta': pasta,
            'TopN_Canais': n,
            'Canais_Usados': ', '.join(canais_n),
            'Dice_SegmentationInterior': dice_interior
        })

df_result = pd.DataFrame(resultados)
df_result.to_csv('dice_tnbc_segmentation_interior_topN.csv', index=False)
print("✅ Resultado salvo em dice_tnbc_segmentation_interior_topN.csv")
